...
class DigitSpan:
    ...
    def __init__(self, window, level, dual_task):
        ...
        if EXPERIMENT:
            ...
            self.instructions = [
            'In this part of the experiment, you\'ll see a sequence of numbers, one at a time, in the centre of the screen.\n\nYou will then be asked to repeat the sequence by typing it back into a text box that appears on the screen.\n\n\n\nClick start to begin the practice trials.',
            'Thank you for completing this questionnaire.\n\n\n\nThis is the final part of the experiment.\n\nIn this part you will complete the same type of digit sequence trials, but in this part, there will be a secondary task between seeing and repeating the sequence.\n\n\n\nThe first trial is a practice trial\n\nClick start to begin the practice trial.',
            'Thank you, the practice trial is now complete.\n\n\nDoes it make sense what you have to do?\n\n\nClick start to begin.']
        else:
            self.instructions = [
            'In this part of the experiment, you\'ll see a sequence of numbers, one at a time, in the centre of the screen.\n\nYou will then be asked to repeat the sequence by typing it back into a text box that appears on the screen.\n\n\n\nClick start to begin the practice trials.',
            'Thank you for completing this questionnaire.\n\n\n\nThis is the final part of the experiment.\n\nIn this part you will complete the same type of digit sequence trials, but in this part, the gap between seeing and repeating the sequence will be longer.\n\n\n\nThe first trial is a practice trial\n\nClick start to begin the practice trial.',
            'Thank you, the practice trial is now complete.\n\n\nDoes it make sense what you have to do?\n\n\nClick start to begin.']
    ...
    def start(self):
        """Changes made: Changed the number of trials before an instruction."""
        try:
            self.stimulus_label.destroy()
        except:
            pass
        self.start_button = Button(self.window, text='Start', command=self.create)
    ...
    def create(self):
        ...
        self.digits = [str(x) for x in self.digits]
        self.digits = ''.join(map(str, self.digits))
        self.digits = list(self.digits)
        while len(self.digits) != self.level:
            self.digits.pop()
        self.check_triplicates(self.digits)

    def check_triplicates(self, digit_list):
        counts = dict()
        for i in digit_list:
        ...
        if 3 in values or 4 in values or 5 in values:
            ...
            self.check_neighbours(digit_list)

    def check_neighbours(self, digit_list):
        self.digit_check = [int(x) for x in digit_list]
        self.result = [x - y for x, y in self.pairwise(self.digit_check)]
        if 0 in self.result:
            self.create()
        else:
            self.insert_crosses(self.digit_check)

    def insert_crosses(self, digit_list):
        self.digits = '+'.join(map(str, digit_list))
        self.digits_answer = ''.join(map(str, self.digits))
        self.interstimulus_cross()
    ...
    def interstimulus_cross(self):
        ...
        self.stimulus_label = Label(self.window, text='+', bg='White',
                                    font=('Arial', int(WINDOW_HEIGHT / font(SCREEN_HEIGHT, 65))),
                                    fg='black')
        self.stimulus_label.place(relx=.5, rely=.5, anchor=CENTER)
        self.window.after(two_seconds, self.digit_update)

    def digit_update(self):
        try:
            self.stimulus_label.config(text=f'{self.digits[self.digit_index]}', fg='black')
            if self.digit_index % 2 == 0:
                self.stimulus_label.config(font=('Arial', int(WINDOW_HEIGHT / font(SCREEN_HEIGHT, 65))))
            else:
                self.stimulus_label.config(font=('Arial', int(WINDOW_HEIGHT / font(SCREEN_HEIGHT, 30))))
            self.digit_index += 1
            self.window.update()
            self.window.after(two_hundred_milliseconds, self.digit_update)
        except IndexError:
    ...
    def titration_trial(self):
        if not self.dual_task:
            self.stimulus_label.config(text='Repeat the Sequence',
                                       font=('Arial', int(WINDOW_HEIGHT / font(SCREEN_HEIGHT, 30))), fg='black')
            self.window.update()
    ...
    def dual_task_trial(self):
        self.stimulus_label.place_forget()
        if self.practice:
    ...
    def response(self):
        ...
        # if not self.dual_task:
        #     self.stimulus_label.destroy()
        if self.dual_task and self.practice and EXPERIMENT:
        ...
        self.stimulus_label.config(text='Repeat the Sequence',
                                   ...
                                   fg='black')
        self.stimulus_label.place(relx=.5, rely=.35, anchor=CENTER)
        self.entry = Entry(self.window, justify='center', relief='sunken', width=20, font=('default', int(WINDOW_HEIGHT / font(SCREEN_HEIGHT, 20))))
    ...
    def response_check(self):
        ...
        self.response_input = self.entry.get()
        self.entry.destroy()
        self.stimulus_label.destroy()
        ...
        if not self.dual_task:
            if self.practice and len(self.titration_trial_list) < 2:
                ...
                self.level_up()
            else:  # During titration trials
                self.digits_answer = ''.join(map(str, self.digits))
                self.digits_answer = self.digits_answer.replace('+', '')
                if self.response_input == self.digits_answer:
                ...
                if len(self.intra_trial_outcomes) == 2 and sum(self.intra_trial_outcomes) == 2:
                    ...
                    RESPONSES.append(f'Digit Trials: {len(self.titration_trial_list) - 2}')
            ...
            self.trial_response = self.response_input
            self.trial_response = list(self.trial_response)
            self.trial_target = self.digits_answer.replace('+', '')
            self.trial_target = list(self.trial_target)
            ...
            RESPONSES.extend([f'Target: {str(self.trial_target)} Response: {str(self.trial_response)} Outcome: {str(self.trial_outcome)}', self.trial_outcome_percent])
    ...
    def time_question_handler(self, _):
        ...
        self.window.update()
        if self.dual_task_counter == 6:
            recall_phase = QuestionPhase(root)
            recall_phase.instructions()
        else:
            self.next_trial()
...
class VisualLanguageFluencyIndex:
    ...
    def calculate(self):
        ...
        DigitSpan(root, RESPONSES[8], True).start()
