...
class InformationSheet:
    ...
    def submit(self):  # Demographic entry checks
        ...
        if self.age.get() in self.illegals:
            ...
            VisualPatternTest(root, 2, False).start()
...
class VisualPatternTest:
    ...
    def __init__(self, window, level, dual_task):
        ...
        if EXPERIMENT:
            ...
            self.instructions = [
                'In this part of the experiment, you\'ll see a grid of black and white squares.\n\nThe grid will turn blank and you will be asked to fill the pattern back into the grid.\n\n\n\nThe number of blocks in the grid will slowly increase with time.\n\n\n\nClick start to begin the practice trials.',
                'Thank you for completing this questionnaire.\n\n\n\nThis is the final part of the experiment.\n\nIn this part you will complete the same type of grid trials, but in this part, there will be a secondary task between seeing and repeating the sequence.\n\n\n\nThe first trial is a practice trial.\n\nClick start to begin the practice trial.',
                'Thank you, the practice trial is now complete.\n\n\nDoes it make sense what you have to do?\n\n\nClick start to begin.']
        else:
            self.instructions = [
                'In this part of the experiment, you\'ll see a grid of black and white squares.\n\nThe grid will turn blank and you will be asked to fill the pattern back into the grid.\n\n\n\nThe number of blocks in the grid will slowly increase with time.\n\n\n\nClick start to begin the practice trials.',
                'Thank you for completing this questionnaire.\n\n\n\nThis is the final part of the experiment.\n\nIn this part you will complete the same type of grid trials, but this time, the gap between seeing and filling in will be longer.\n\n\n\nThe first trial is a practice trial.\n\nClick start to begin the practice trial.',
                'Thank you, the practice trial is now complete.\n\n\nDoes it make sense what you have to do?\n\n\nClick start to begin.']
    ...
    def colour_change(self, index):
        """Changes the colours of the visual pattern blocks"""
        if index['bg'] == '#ffffff':
            index['bg'] = 'black'
    ...
    def start(self):
        self.start_button = Button(root, text='Start', command=self.interstimulus_cross)
        ...
        if not self.dual_task and len(self.titration_trial_list) == 0:
            self.instructions_label.config(text=self.instructions[0], wraplength=800,
                                           ...
                                           justify='center')  # Titration Pre Instructions
        ...
        # Titration Post Instructions
            ...
            self.instructions_label.config(text=self.instructions[2], wraplength=800,
                                           font=('Arial', int(WINDOW_HEIGHT / font(SCREEN_HEIGHT, 15)),),
                                           justify='center')  # Titration Post Instructions
            self.instructions_label.place(relx=.5, rely=.5, anchor=CENTER)
            self.start_button.place(relx=0.5, rely=0.85, anchor=CENTER)
        # Dual Task Pre Instructions
            ...
            self.instructions_label.config(text=self.instructions[1], wraplength=800,
                                           font=('Arial', int(WINDOW_HEIGHT / font(SCREEN_HEIGHT, 15))),
                                           justify='center')  # Dual Task Pre Instructions
            self.instructions_label.place(relx=.5, rely=.5, anchor=CENTER)
            self.start_button.place(relx=0.5, rely=0.85, anchor=CENTER)
        # Dual Task Post Instructions
        elif self.practice and len(self.practice_list) == 1:
            try:
            ...
            self.instructions_label.config(text=self.instructions[2], wraplength=800,
                                           font=('Arial', int(WINDOW_HEIGHT / font(SCREEN_HEIGHT, 15))),
                                           justify='center')  # Dual Task Post Instructions
            self.instructions_label.place(relx=.5, rely=.5, anchor=CENTER)
            self.start_button.place(relx=0.5, rely=0.85, anchor=CENTER)
        else:
    ...
    def interstimulus_cross(self):
        ...
        self.interstimulus_cross_label = Label(root, font=('Arial', int(WINDOW_HEIGHT / font(SCREEN_HEIGHT, 65))), text='+', bg='white')
        self.interstimulus_cross_label.place(relx=.5, rely=.5, anchor=CENTER)
        self.window.update()
        self.window.after(two_seconds, self.block_setup)

    def block_setup(self):
        self.interstimulus_cross_label.destroy()
        # self.block_canvas = Canvas(root, width=WINDOW_WIDTH + 10, height=WINDOW_HEIGHT + 10, highlightthickness=0, bg='red')
        # self.block_canvas.pack()
        self.block_list = []
        self.block_frame_list = []
        self.enclosing_frame = Frame(root, bg='white')

        for x in range(self.level*2): # for every n in the range of level * 2
            self.block_frames = Frame(self.enclosing_frame, highlightthickness=2, highlightbackground="gray", bg='white') # Create a frame
            self.block_frame_list.append(self.block_frames)  # append the frame to block_frame_list
            self.blocks = Button(self.block_frames, width=int(WINDOW_WIDTH * 0.038), height=int(WINDOW_WIDTH * 0.038), bg='#ffffff', bd=4, highlightthickness=2, focuscolor='') # Put a button inside the frame
    ...
    def presentation(self):
        ...
        self.window.update()
        self.window.after(two_seconds, self.trial_selection)
    ...
    def titration_trial(self):
        ...
        if not self.dual_task:
            self.instruction_label = Label(root, text='Fill in the pattern', font=('Arial', 20), bg='white')
            self.instruction_label.place(relx=.5, rely=.5, anchor=CENTER)
            self.window.update()
            self.titration_trial_list.append(1)
            self.window.after(two_seconds, self.response)
        elif self.dual_task:
            if self.practice:
                self.practice_list.append(1)
            self.instruction_label = Label(root, text='+', font=('Arial', 20), bg='white')
            self.instruction_label.place(relx=.5, rely=.5, anchor=CENTER)
            self.window.update()
            self.dual_task_counter += 1
            self.window.after(seven_seconds, self.response)
    ...
    def response(self):
        ...
        if not self.dual_task or CONTROL and self.dual_task:
        ...
        self.enclosing_frame.place(relx=.5, rely=.5, anchor=CENTER)
        self.window.bind('<Return>', self.response_check)
        self.window.update()

    def response_check(self, _):
        """Get trial target
        Trial target is pattern_list
        Get trial response
        check_list is a list of of the blocks by number that have been returned black.
        What is the format of these lists?
        Work out percentage accuracy
        Append the response and outcome to RESPONSES
        Append the percentage accuracy to RESPONSES"""
        self.window.unbind('<Return>')
        self.window.update()
        self.check_list = [block for block in self.block_list if block['bg']=='black']
        self.enclosing_frame.destroy()
        if not self.dual_task:
            if self.practice and len(self.titration_trial_list) < 2:
                ...
                if len(self.intra_trial_outcomes) == 2 and sum(self.intra_trial_outcomes) == 2:
                    ...
                    RESPONSES.append(f'VPT Trials: {len(self.titration_trial_list) - 2}')
                    RESPONSES.append(int(self.level - 1))
                    VisualLanguageFluencyIndex(root).start()
                else:
                    self.next_trial()

        elif self.dual_task:
            self.target_strings = [str(x) for x in self.pattern_list]
            self.target = []
            for element in self.target_strings:
                self.target.append(element[15:17])

            for i in range(len(self.target)):
                if '.!' in self.target[i]:
                    self.target[i] = self.target[i].replace('.!', '1')
                if '.' in self.target[i]:
                    self.target[i] = self.target[i].replace('.', '')

            self.response_strings = [str(x) for x in self.check_list]
            self.participant_response = []
            for element in self.response_strings:
                self.participant_response.append(element[15:17])

            for i in range(len(self.participant_response)):
                if '.!' in self.participant_response[i]:
                    self.participant_response[i] = self.participant_response[i].replace('.!', '1')
                if '.' in self.participant_response[i]:
                    self.participant_response[i] = self.participant_response[i].replace('.', '')

            check_set = set(self.participant_response) # Creates a set of the check list (removes duplicates and removes order)
            pattern_set = set(self.target) # Creates a set of the check list (removes duplicates and removes order)

            intersection = check_set.intersection(pattern_set)
            union = check_set.union(pattern_set)

            accuracy = len(intersection) / len(union)
            RESPONSES.extend([f'Target: {self.target} Response: {self.participant_response}', accuracy])
    ...
    def time_question_display(self):
        # self.time_question_canvas = Canvas(root, width=WINDOW_WIDTH + 15, height=WINDOW_HEIGHT + 15, bg='white')
        self.time_question_label = Label(root, text=self.time_q, background='white',
                                         font=('Arial', int(WINDOW_HEIGHT / font(SCREEN_HEIGHT, 20))))
...
class VisualLanguageFluencyIndex:
    ...
    def calculate(self):
        ...
        if FAST_COMPONENT_DEMO or FULL_COMPONENT_DEMO or component == '2':
            return sys.exit()

        VisualPatternTest(root, RESPONSES[8], True).start()
...
class QuestionPhase:
    ...
    def __init__(self, window):
        ...
        self.content_prompt = Label(self.content_canvas, text='What happened in the comic?', font=('default', int(WINDOW_HEIGHT / font(SCREEN_HEIGHT, 20))), bg='white')
        ...
        for _ in range(0, 6):
            ...
            self.descriptions_entry_box = Entry(self.content_canvas, relief='sunken', borderwidth=2, font=("Arial", int(WINDOW_HEIGHT / font(SCREEN_HEIGHT, 24))), width=int(WINDOW_WIDTH * 0.015), textvariable=self.descriptions_var)
        ...
        for _ in range(0, 6):
            ...
            self.panel_entry_box = Entry(self.panels_canvas, justify='center', relief='sunken', borderwidth=2, font=("Arial", int(WINDOW_HEIGHT / font(SCREEN_HEIGHT, 24))), width=int(WINDOW_WIDTH * 0.003), textvariable=self.panels_var)
    ...
    def instructions(self):
        self.instructions_label = Label(root, text=self.free_recall_instructions, font=('default', int(WINDOW_HEIGHT / font(SCREEN_HEIGHT, 20))), bg='white')
        self.instructions_label.place(relx=.5, rely=.5, anchor=CENTER)
...
if DEV_MODE or FAST_COMPONENT_DEMO or FULL_COMPONENT_DEMO:
    ...
    components_dict = {
        ...
        '2': lambda: VisualPatternTest(root, 2, False).start(),
        '3': lambda: VisualLanguageFluencyIndex(root).start(),
        '4': lambda: VisualPatternTest(root, 9, True).start(),
